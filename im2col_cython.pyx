...
def im2col_cython(np.ndarray[DTYPE_t, ndim=4] x, int field_height, int field_width, int padding, int stride):
    ...
    cdef int H = x.shape[3]

    cdef int HH = (H + 2 * padding - field_height) / stride + 1
    cdef int WW = (W + 2 * padding - field_width) / stride + 1
    cdef int P = padding
    cdef np.ndarray[DTYPE_t, ndim=4] x_pad = np.pad(x, ((0, 0), (0, 0), (P, P), (P, P)), mode='constant', constant_values=0)
    cdef np.ndarray[DTYPE_t, ndim=2] cols = np.zeros((C*field_height*field_width, N*HH*WW), dtype = x.dtype)
    im2col_cython_inner(cols, x_pad, N, C, H, W, HH, WW, field_height, field_width, padding, stride)
    return cols

cdef int im2col_cython_inner(np.ndarray[DTYPE_t, ndim=2] cols, np.ndarray[DTYPE_t, ndim=4] x_pad, int N, int C, int H, int W, int HH, int WW, \
            ...
            for xx in range(WW):
                for ii in range(field_height):
                    for jj in range(field_width):
                        ...
                        for i in range(N):
                            col = yy * WW * N + xx * N + i
                            cols[row, col] = x_pad[i, c, stride * yy + ii, stride * xx + jj]
...
cdef col2im_6d_cython_inner(np.ndarray[DTYPE_t, ndim = 6] cols, np.ndarray[DTYPE_t, ndim = 4] x_padded, int N, \
                                int C, int H, int W, int HH, int WW, int out_h, int out_w, int pad, int stride):
